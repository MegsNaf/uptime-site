...
client.on("ready", () => {
  client.user.setActivity(
    `maded by 𝐱𝐂𝐋𝐀𝐙𝐘#7777 and samet i kelam / rent a car#1077`
  );
  passport.use(
    new Strategy(
      {
  ...
  app.get("/autherror", (req, res) => {
    res.send(
      "Giriş Hatası Lütfen Tekrar Dene : https://al-uptime.glitch.me/"
    );
  });
  ...
  app.post("/add", checkAuth, (req, res) => {
    ...
    if (
      ...
      return res.send("Bu Proje Sistemde Zaten Mevcut!");
    } else {
      db.push("linkler", { url: link, owner: req.user.id });
      res.send("Projen Eklendi ID= " + req.user.id);
    }
  });
